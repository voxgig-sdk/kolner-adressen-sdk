# KolnerAdressen Lua SDK Reference

Complete API reference for the KolnerAdressen Lua SDK.
...
```lua
local results, err = client:Address(nil):list(nil, nil)
```
...
```lua
local result, err = client:DatastoreSearch(nil):load({ id = "datastore_search_id" }, nil)
```
