# KolnerAdressen Lua SDK



The Lua SDK for the KolnerAdressen API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Address()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/kolner-adressen-sdk/releases)),
...
Create an instance: `local datastore_search = client:DatastoreSearch(nil)`
...
```
lua/
├── kolner-adressen_sdk.lua    -- Main SDK module
...
The main module (`kolner-adressen_sdk`) exports the SDK constructor
