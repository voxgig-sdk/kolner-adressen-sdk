# KolnerAdressen Ruby SDK



The Ruby SDK for the KolnerAdressen API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Address` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = KolnerAdressenSDK.test(testopts, sdkopts)
```
...
raise a `KolnerAdressenError` (a `StandardError` subclass), so wrap
...
```
rb/
├── KolnerAdressen_sdk.rb       -- Main SDK module
...
The main module (`KolnerAdressen_sdk`) exports the SDK class
