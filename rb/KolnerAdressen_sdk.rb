...
require_relative 'KolnerAdressen_types'
...
class KolnerAdressenSDK
  ...
  def initialize(options = {})
    ...
    utility = KolnerAdressenUtility.new
    @_utility = utility
    ...
    feature_opts = KolnerAdressenHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = KolnerAdressenHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, KolnerAdressenFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    KolnerAdressenUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = KolnerAdressenHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = KolnerAdressenHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = KolnerAdressenHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = KolnerAdressenSpec.new({
  ...
  def raw_request(fetchargs = {})
    ...
    rescue KolnerAdressenError => err
    ...
    if fetched.is_a?(Hash)
      status = KolnerAdressenHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = KolnerAdressenError.new(
        "graphql_error", "KolnerAdressenSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = KolnerAdressenSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
