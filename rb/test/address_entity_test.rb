...
class AddressEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = KolnerAdressenSDK.test(seed, nil)
    seen = base.Address(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = KolnerAdressenConfig.make_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = KolnerAdressenSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KOLNER_ADRESSEN_TEST_ADDRESS_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def address_basic_setup(extra)
  ...
  entid_env_raw = ENV["KOLNER_ADRESSEN_TEST_ADDRESS_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["KOLNER_ADRESSEN_TEST_ADDRESS_ENTID"])
