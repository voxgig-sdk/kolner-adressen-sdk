...
[MCP](https://modelcontextprotocol.io) server exposing the KolnerAdressen SDK as
two agent tools — `kolner-adressen_list` and `kolner-adressen_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kolner-adressen-mcp)
make build

# 2. Provide credentials via the environment
export KOLNER_ADRESSEN_APIKEY=sk_live_xxx
...
```jsonc
// kolner-adressen_list: first page of records
{ "entity": "address" }
{ "entity": "address", "query": { } }

// kolner-adressen_load: one record by id
{ "entity": "datastore_search", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kolner-adressen-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kolner-adressen \
     -- "$PWD"/dist/*/kolner-adressen-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kolner-adressen_list` and `kolner-adressen_load` tools now appear
   in new sessions. Ask the agent to *"list address using kolner-adressen"*
   and it calls `kolner-adressen_list` with `{"entity":"address"}`.
...
```sh
export KOLNER_ADRESSEN_APIKEY=sk_live_xxx            # API key
export KOLNER_ADRESSEN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `kolner-adressen_list` tool
...
### Call the `kolner-adressen_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `kolner-adressen_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kolner-adressen_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KOLNER_ADRESSEN_APIKEY` | API key sent with every request. |
| `KOLNER_ADRESSEN_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"kolner-adressen_load","arguments":{"entity":"datastore_search","query":{"id":1}}}}'
```
