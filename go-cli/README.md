...
boru-driven command-line client **and** interactive REPL for the KolnerAdressen
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kolner-adressen-cli)
make build

# 2. See usage (words, entities, env vars)
./kolner-adressen-cli --help

# 3. Provide credentials once, via the environment
export KOLNER_ADRESSEN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./kolner-adressen-cli list address
./kolner-adressen-cli list datastore_search

# 5. Override the API base URL for a single call
KOLNER_ADRESSEN_BASE=https://api.example.com ./kolner-adressen-cli list address

# 6. No arguments -> interactive REPL
./kolner-adressen-cli
kolner-adressen> list address
kolner-adressen> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kolner-adressen-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/kolner-adressen-cli list address
   ```
...
Run with no arguments to open a REPL (prompt `kolner-adressen>`). Each line is
evaluated as its own boru expression:

```text
$ ./kolner-adressen-cli
kolner-adressen> list address
kolner-adressen> /help
kolner-adressen> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kolner-adressen-cli`. |
...
(`./kolner-adressen-cli <expr>`) and interactively (the REPL), and expressions compose the
