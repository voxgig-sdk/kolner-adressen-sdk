...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.KOLNERADRESSEN_TEST_LIVE ||
    'TRUE' === process.env.KOLNERADRESSEN_TEST_OVERRIDE
  ) {
  ...
  m.KOLNERADRESSEN_TEST_EXPLAIN = process.env.KOLNERADRESSEN_TEST_EXPLAIN || m.KOLNERADRESSEN_TEST_EXPLAIN
