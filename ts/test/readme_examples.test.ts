...
//     (\`new KolnerAdressenSDK(...)\` / \`KolnerAdressenSDK.test(...)\` become
//     \`KolnerAdressenSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'KolnerAdressenSDK'
...
const TEST_SEED = {"entity":{"address":{"test01":{"id":"test01"}},"datastore_search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KolnerAdressenSDK(...)\` and any
// \`KolnerAdressenSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KolnerAdressenSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KolnerAdressenSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
