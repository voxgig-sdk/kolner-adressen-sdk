...
The TypeScript SDK for the KolnerAdressen API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const result = await client.Address().list()
...
```ts
const client = new KolnerAdressenSDK({ apikey: '...' })
