...
module KolnerAdressenUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KolnerAdressenResponse.new({})
    ...
    if fetch_err
      ...
      response = KolnerAdressenResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KolnerAdressenResponse.new(fetched)
    else
