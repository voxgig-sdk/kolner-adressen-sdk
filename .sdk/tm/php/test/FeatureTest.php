...
// KolnerAdressen SDK feature test
//
...
class FtCtrl extends KolnerAdressenControl
{
...
class FtClient
{
    ...
    private KolnerAdressenUtility $utility;

    public function __construct(KolnerAdressenUtility $utility, array $options)
    {
    ...
    public function get_utility(): KolnerAdressenUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KolnerAdressenContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new KolnerAdressenUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (KolnerAdressenContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KolnerAdressenConfig::make_feature($name);
    ...
    private static function buildUrl(KolnerAdressenSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KolnerAdressenError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KolnerAdressenResponse($fetched);
        }
    ...
    private function fail(KolnerAdressenContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KolnerAdressenContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
