...
class KolnerAdressenMakeError
{
    public static function call(?KolnerAdressenContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new KolnerAdressenContext([], null);
        }
        $op = $ctx->op ?? new KolnerAdressenOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new KolnerAdressenResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof KolnerAdressenError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new KolnerAdressenError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof KolnerAdressenError) {
