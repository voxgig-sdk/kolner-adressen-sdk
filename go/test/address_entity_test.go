...
func TestAddressEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set KOLNERADRESSEN_TEST_ADDRESS_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func addressBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KOLNERADRESSEN_TEST_ADDRESS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["KOLNERADRESSEN_TEST_ADDRESS_ENTID"])
