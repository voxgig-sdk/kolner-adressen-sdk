...
// KolnerAdressen SDK retry feature
...
class KolnerAdressenRetryFeature extends KolnerAdressenBaseFeature
{
    ...
    public function init(KolnerAdressenContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (KolnerAdressenContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(KolnerAdressenContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(KolnerAdressenContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
