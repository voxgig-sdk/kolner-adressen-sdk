...
// KolnerAdressen SDK DatastoreSearch entity
...
class DatastoreSearchEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DatastoreSearchEntity($this->_client, $opts);
    }

    /**
     * @param DatastoreSearch|array $args DatastoreSearch data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = KolnerAdressenHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return DatastoreSearch|array The current DatastoreSearch data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of DatastoreSearch fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = KolnerAdressenHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of DatastoreSearch fields).
     */
    ...
    /**
     * Load a single DatastoreSearch.
     *
     * @param DatastoreSearchLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed DatastoreSearchLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return DatastoreSearch|array The loaded DatastoreSearch as an assoc-array at the
     *   SDK boundary; throws KolnerAdressenError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = KolnerAdressenHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
