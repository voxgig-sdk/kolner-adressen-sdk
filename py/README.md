# KolnerAdressen Python SDK

The Python SDK for the KolnerAdressen API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Address(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.KolnerAdressen(None).load(
    {"id": "test01"}, None
)
...
```ts
const datastore_search = await client.DatastoreSearch().load({ id: 'datastore_search_id' })
```
...
```
py/
├── kolneradressen_sdk.py         -- Main SDK module
