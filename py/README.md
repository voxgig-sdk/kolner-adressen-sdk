...
The Python SDK for the KolnerAdressen API — an entity-oriented client following Pythonic conventions.
...
```python
result, err = client.Address().list()
if err:
    raise Exception(err)
...
result, err = client.KolnerAdressen().load({"id": "test01"})
