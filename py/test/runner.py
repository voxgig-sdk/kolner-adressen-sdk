# KolnerAdressen SDK test runner
...
class KolnerAdressenTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KolnerAdressenTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KolnerAdressenTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KolnerAdressenTestRunner.getenv("KOLNER_ADRESSEN_TEST_LIVE")
        override = KolnerAdressenTestRunner.getenv("KOLNER_ADRESSEN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KolnerAdressenTestRunner.getenv(key)
        ...
        explain = KolnerAdressenTestRunner.getenv("KOLNER_ADRESSEN_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if KolnerAdressenTestRunner._test_control is not None:
            return KolnerAdressenTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KolnerAdressenTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KolnerAdressenTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KolnerAdressenTestRunner.load_test_control()
...
def load_env_local():
    KolnerAdressenTestRunner.load_env_local()


def env_override(m):
    return KolnerAdressenTestRunner.env_override(m)


def entity_data(v):
    return KolnerAdressenTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KolnerAdressenTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KolnerAdressenTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KolnerAdressenTestRunner.load_test_control()


def live_delay_ms():
    return KolnerAdressenTestRunner.live_delay_ms()
